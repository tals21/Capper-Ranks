#!/usr/bin/env python3
"""
Test script to fetch and process a tweet from DanGambleAI for image processing.
This script will help test the OCR and pick detection functionality with real data.
"""
...
def test_dangambleai_tweet():
    """Test image processing with a tweet from DanGambleAI."""
    print("=== Testing Image Processing with DanGambleAI Tweet ===\n")
    ...
    # Get DanGambleAI user info
    print("🔍 Looking up DanGambleAI...")
    user = x_client.get_user_from_username(client, "DanGambleAI")
    if not user:
        print("❌ Could not find user @DanGambleAI")
        return False
    ...
    if not tweets_with_media:
        print("❌ No tweets with media found from DanGambleAI")
        return False
...
def main():
    ...
    print("=== DanGambleAI Image Processing Test ===\n")
